...
function setupGame() {
  ...
  // State
  progressionState = GAME_PROGRESSION.ONE;
...
function draw() {  
  ...
  if (started) {
    currentState = GAME_STATE.STARTED;
    hr.show(hr);
      ...
      if (enemiesSmall[i].checkBounds()) {
        ...
        if (hr.intersects(enemiesSmall[i])) {
          ...
          if (enemiesSmall[i] != undefined) {
              hr.attackResult(enemiesSmall[i].health, progressionState);
              if(hr.getHealth() > 0) {
                gameOver();
              } else {
                return;
      ...
      if (enemiesMedium[i].checkBounds()) {
        ...
        if (hr.intersects(enemiesMedium[i])) {
          ...
          if (enemiesMedium[i] != undefined) {
              hr.attackResult(enemiesMedium[i].health, progressionState);
              if(hr.getHealth() > 0) {
                gameOver();
              }
          }
      ...
      if (enemiesBig[i].checkBounds()) {
        ...
        if (hr.intersects(enemiesBig[i])) {
          ...
          if (enemiesBig[i] != undefined) {
              hr.attackResult(enemiesBig[i].health, progressionState);
              if(hr.getHealth() > 0) {
                gameOver();
              }
          }
    ...
    // Game State Switch
    switch(currentState) {
      case GAME_STATE.PAUSED : break;
      case GAME_STATE.STARTING : break;
      case GAME_STATE.STARTED : break;
      case GAME_STATE.GAME_OVER : gameOver(); break;
    }
  }
}
